...
export const BOUNDARY_CONTRAST_LIMITS = {
	...
	harmonicWindow: 0.1,
	/**
	 * 粗い走査が素通りした境界コントラストの山を、いくつまで再走査するか。
	 * [Intended] 1 個では足りない。正解の 1 オクターブ下が全域の最大になる入力があり
	 * （実測: 540x540・セル 30px の市松で出力 9 の証拠 31.7 が最大、正解 18 はその次）、
	 * 正解の山は 2 番目以降に回る。多くしても採否は再構成誤差が決めるので害は無いが、
	 * 山 1 つにつき再走査が 1 回増えるため、探索時間との釣り合いでここに留める。
	 */
	skippedPeakCandidates: 3,
	/**
