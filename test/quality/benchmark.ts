...
const renderHtml = (results: QualityResults): string => {
	...
	<header><h1>PixelRefiner quality report</h1><p>PR ${escapeHtml(results.metadata.prNumber)} · head ${escapeHtml(results.metadata.headCommit)} · base ${escapeHtml(results.metadata.baseCommit)} · ${escapeHtml(results.metadata.generatedAt)} · <a href="${escapeHtml(results.metadata.workflowRunUrl)}">workflow</a> · benchmark v${results.metadata.benchmarkVersion} · report v${results.metadata.reportVersion}</p><input id="search" placeholder="Filter cases"><select id="status"><option value="">All statuses</option><option>passed</option><option>failed</option></select></header>
