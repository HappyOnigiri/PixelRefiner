...
- **アウトライン生成**: スプライトに自動で縁取り（アウトライン）を追加します。
  ...
  - **色指定**: 任意の色で縁取り可能。
- **プリセット管理**: お気に入りの設定を保存・読み込みして、異なる画像で再利用できます。
...
5. 必要に応じて設定パネルで以下のパラメータを調整できます：
    - **グリッド検出**: 検出モードの選択（自動/Hint/Force/無効）、候補の選択、高速モードの切り替え
...
7. 複数枚ある場合は、**「一括ダウンロード (ZIP)」** ボタンから全ての画像をまとめてエクスポートできます。
