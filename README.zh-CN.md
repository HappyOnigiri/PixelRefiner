...
- **描边生成**：自动给 sprite 添加描边。
  ...
  - **自定义颜色**：可选择任意描边颜色。
- **以 Auto 为中心的设置**：通过处理方式、细节、颜色、背景、抖动和轮廓等用途导向选项开始，技术参数仍可在高级设置中使用。
- **用途预设**：可选择 Auto、清晰精灵、保留细节、透明图标、限制颜色或照片转像素画，也可保存自定义配置。
- **自动裁剪**：移除透明边距并裁剪到内容边界。
- **强制缩放**：按指定像素宽高输出。
...
5. 选择用途预设，或调整**处理方式**、**细节**、**颜色**、**背景**、**抖动**、**轮廓**和**自动裁剪**。
6. 只有在需要精确控制网格、调色板、容差、连接方式或强制尺寸时才打开**高级设置**。
7. 在结果尺寸旁查看输入分类、采用的 route 和可信度。只有网格可信度较低时，才会在结果下方列出其他像素尺寸。
8. 使用 **Compare** 视图通过滑块比较原图和处理结果。
9. 满意后点击 **Download** 保存结果，也可以通过下拉按钮选择放大倍率。
10. 多张图片时，使用 **Download All (ZIP)** 一次导出所有已处理 sprite。
...
或在结果不确定时安全地保留原始尺寸。网格判断置信度较低时，应用会在结果下方列出
其他像素尺寸，而不会强制进行极端缩小。
