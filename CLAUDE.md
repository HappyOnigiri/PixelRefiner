...
## Commands & CI
- **Dev Server**: `pnpm run dev` (`--host 0.0.0.0` available).
...
- **Tests**: Write logic tests in `src/core/*.test.ts`. Test fixtures are in `test/fixtures/`. Set `PIXELATE_DEBUG_IMAGES=1 pnpm test` to output debug images to `tmp/debug`.
